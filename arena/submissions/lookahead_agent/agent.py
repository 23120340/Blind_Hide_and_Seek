"""
Depth-limited lookahead agents for Blind Hide and Seek.

This variant uses shallow adversarial simulation when the opponent is visible:
Pacman maximizes the worst post-move capture position, while Ghost maximizes
its worst-case escape distance against Pacman's straight-line speed.
"""
...
class GhostAgent(BaseGhostAgent, GridTools):
    ...
    def _safe_cell_score(self, pos, threat):
        ...
        score = exits * 8 + self._unknown_count(pos, self.known_map) * 2
        score -= self.visited.get(pos, 0) * 2
        if pos in list(self.recent)[-4:]:
            score -= 5
        if exits <= 1:
            score -= 45
        if threat is not None:
            score += min(25, self._maze_distance(pos, threat, self.known_map, max_depth=25)) * 4
        return score
