"""
Fixed-map Voronoi/articulation agent.

Pacman uses fast frontier/BFS pursuit with the fixed-map prior to beeline
toward the known ghost spawn on the default map.

Ghost uses a blended safety evaluator combining four signals:
...
class PacmanAgent(BasePacmanAgent, ArenaTools):
    def __init__(self, **kwargs):
        ...
        self.name = "Fixed Voronoi Frontier Hunter"
...
class GhostAgent(BaseGhostAgent, ArenaTools):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.name = "Fixed Voronoi Escape Hider"
        self._init_memory()
        self.fixed_opening = [
            Move.RIGHT,
            Move.RIGHT,
