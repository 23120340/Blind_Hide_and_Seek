"""
Experimental seek predictor agent.
...
class PacmanAgent(BasePacmanAgent, ArenaTools):
    def __init__(self, **kwargs):
        ...
        self.name = "Seek Predictor Trap Seeker"
        self.pacman_speed = max(1, int(kwargs.get("pacman_speed", 1)))
        self._init_memory()
        self.predicted_opening = [
            Move.RIGHT,
            Move.RIGHT,
            Move.RIGHT,
            Move.UP,
    ...
    def step(self, map_state, my_position, enemy_position, step_number):
        ...
        elif self.fixed_map_mode and step_number <= len(self.predicted_opening) + 3:
            target = self._predicted_opening_target(step_number)
        else:
    ...
    def _predicted_opening_target(self, step_number):
        pos = DEFAULT_GHOST_START
        # Aim slightly ahead because Pacman moves simultaneously and has speed 2.
        horizon = min(len(self.predicted_opening), step_number + 2)
        for move in self.predicted_opening[:horizon]:
            nxt = self._move(pos, move)
            if not self._open(nxt, self.known):
                break
            pos = nxt
